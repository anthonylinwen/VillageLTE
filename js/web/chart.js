...
function get_chart_data(mid, sid, keyword, recommend) {
    ...
    $.ajax({
        ...
        success: function (result) {
			...
			var title_sub1="" ;
			var title_sub2="" ;
			tb += "<ul type='square'><table class='content-table' width='100%' cellpadding='3' cellspacing='0'><tbody>";	
			tb += "<tr>";			
            for (var i = 0; i < result.length; i++) 
			{				
				tb += "<td headers='table_1_0'><li><span>"
				var data = result[i];
				var short_name = data.name;
				var kindname = data.kindname;
			...
			//	html = html.replace("{id}", data.id);
				html = html.replace("{announce-date}",data.pdate );   
			...
			title_sub2="媒體視訊" ;
			$("div .title-sub1").text(kindname);  
			$(".chart-container .title-sub2").text(title_sub2);
			tb += "</tbody></table></ul>";
            if (result.length == 0) {
